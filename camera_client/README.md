...
`camera_client` 是一个使用 Python 和 OpenCV 实现的摄像头拍照和录制视频工具。通过命令行参数可以指定使用电脑内置摄像头或 iPhone 摄像头进行拍照或录制视频。拍照功能可以每分钟拍摄一张关键帧，并在检测到运动时增加拍摄频率。录制视频功能支持录制指定时长的视频，并保存为 MP4 格式。每张图片都会添加时间戳和序列号，方便后续对图片进行时间序列分析。
...
```bash
pip install opencv-python scikit-image argparse
```
